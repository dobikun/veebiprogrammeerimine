...
{ 
 ...
 include("../functions/functions.php");
    ...
    if (strlen($pass) < 7 or strlen($pass) > 15) $error[] = "Provide a password between 7 and 15 characters!";
    if (strlen($surname) < 3 or strlen($surname) > 20) $error[] = "Specify the name of 3 to 20 characters!";
    if (strlen($name) < 3 or strlen($name) > 15) $error[] = "Specify the name of 3 to 15 characters!";
    if (strlen($patronymic) < 3 or strlen($patronymic) > 25) $error[] = "Specify the Middle of 3 to 25 characters!";
    if (!preg_match("/^(?:[a-z0-9]+(?:[-_.]?[a-z0-9]+)?@[a-z0-9_.-]+(?:\.?[a-z0-9]+)?\.[a-z]{2,5})$/i",trim($email))) $error[] = "”кажите корректный email!";
    if (!$phone) $error[] = "Enter phone number!";
    if (!$address) $error[] = "You must specify the delivery address!";
    
    if($_SESSION['img_captcha'] != strtolower($_POST['reg_captcha'])) $error[] = "Invalid code from the image!";
